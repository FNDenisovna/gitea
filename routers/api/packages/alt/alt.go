...
import (
	...
	alt_service "gitea/services/packages/alt"
)
...
func GetRepositoryInfo(ctx *context.Context) {
	_, pub, err := alt_service.GetOrCreateRepositoryInfo(ctx, ctx.Package.Owner.ID)
	if err != nil {
...
func UploadPackageFile(ctx *context.Context) {
	...
	if setting.Packages.DefaultaltSignEnabled || ctx.FormBool("sign") {
	...
	_, _, err = packages_service.CreatePackageOrAddFileToExisting(
		...
		&packages_service.PackageFileCreationInfo{
			PackageFileInfo: packages_service.PackageFileInfo{
				Filename:     fmt.Sprintf("%s-%s.%s.alt", pck.Name, pck.Version, pck.FileMetadata.Architecture),
				CompositeKey: group,
			},
			Creator: ctx.Doer,
			Data:    buf,
...
func DownloadPackageFile(ctx *context.Context) {
	...
	s, u, pf, err := packages_service.GetFileStreamByPackageNameAndVersion(
		...
		&packages_service.PackageFileInfo{
			Filename:     fmt.Sprintf("%s-%s.%s.alt", name, version, ctx.PathParam("architecture")),
			CompositeKey: ctx.PathParam("group"),
		},
	)
	if err != nil {
...
func DeletePackageFile(webctx *context.Context) {
	...
	err := db.WithTx(webctx, func(ctx stdctx.Context) error {
		...
		pf, err := packages_model.GetFileForVersionByName(
			ctx,
			pv.ID,
			fmt.Sprintf("%s-%s.%s.alt", name, version, architecture),
			group,
		)
		if err != nil {
			return err

...
import (
	...
	alt_model "gitea/models/packages/alt"
	user_model "code.gitea.io/gitea/models/user"
	"code.gitea.io/gitea/modules/json"
	packages_module "code.gitea.io/gitea/modules/packages"
	alt_module "gitea/modules/packages/alt"
...
// The alt registry needs multiple metadata files which are stored in this package.
...
func GetOrCreateKeyPair(ctx context.Context, ownerID int64) (string, string, error) {
	priv, err := user_model.GetSetting(ctx, ownerID, alt_module.SettingKeyPrivate)
	if err != nil && !errors.Is(err, util.ErrNotExist) {
		return "", "", err
	}

	pub, err := user_model.GetSetting(ctx, ownerID, alt_module.SettingKeyPublic)
	if err != nil && !errors.Is(err, util.ErrNotExist) {
		return "", "", err
	}
	...
	if priv == "" || pub == "" {
		...
		if err := user_model.SetUserSetting(ctx, ownerID, alt_module.SettingKeyPrivate, priv); err != nil {
			return "", "", err
		}

		if err := user_model.SetUserSetting(ctx, ownerID, alt_module.SettingKeyPublic, pub); err != nil {
			return "", "", err
		}
	}
...
func generateKeypair() (string, string, error) {
	e, err := openpgp.NewEntity("", "alt Registry", "", nil)
	if err != nil {
		return "", "", err
	}
...
// https://docs.pulpproject.org/en/2.19/plugins/pulp_alt/tech-reference/alt.html#repomd-xml
func buildRepomd(ctx context.Context, pv *packages_model.PackageVersion, ownerID int64, data []*repoData, group string) error {
	type Repomd struct {
		...
		Xmlnsalt string      `xml:"xmlns:alt,attr"`
	...
	if err := xml.NewEncoder(&buf).Encode(&Repomd{
		...
		Xmlnsalt: "http://linux.duke.edu/metadata/alt",
		Data:     data,
	}); err != nil {
		return err
	}
...
// https://docs.pulpproject.org/en/2.19/plugins/pulp_alt/tech-reference/alt.html#primary-xml
func buildPrimary(ctx context.Context, pv *packages_model.PackageVersion, pfs []*packages_model.PackageFile, c packageCache, group string) (*repoData, error) {
	...
	type EntryList struct {
		Entries []*alt_module.Entry `xml:"alt:entry"`
	}

	type Format struct {
		License   string             `xml:"alt:license"`
		Vendor    string             `xml:"alt:vendor"`
		Group     string             `xml:"alt:group"`
		Buildhost string             `xml:"alt:buildhost"`
		Sourcealt string             `xml:"alt:sourcealt"`
		Provides  EntryList          `xml:"alt:provides"`
		Requires  EntryList          `xml:"alt:requires"`
		Conflicts EntryList          `xml:"alt:conflicts"`
		Obsoletes EntryList          `xml:"alt:obsoletes"`
		Files     []*alt_module.File `xml:"file"`
	}
	...
	for _, pf := range pfs {
		...
		files := make([]*alt_module.File, 0, 3)
		...
		packages = append(packages, &Package{
			...
			Format: Format{
				...
				Sourcealt: pd.FileMetadata.Sourcealt,
				Provides: EntryList{
...
// https://docs.pulpproject.org/en/2.19/plugins/pulp_alt/tech-reference/alt.html#filelists-xml
...
// https://docs.pulpproject.org/en/2.19/plugins/pulp_alt/tech-reference/alt.html#other-xml
func buildOther(ctx context.Context, pv *packages_model.PackageVersion, pfs []*packages_model.PackageFile, c packageCache, group string) (*repoData, error) { //nolint:dupl
	...
	type Package struct {
		...
		Architecture string                  `xml:"arch,attr"`
		Version      Version                 `xml:"version"`
		Changelogs   []*alt_module.Changelog `xml:"changelog"`
	}
